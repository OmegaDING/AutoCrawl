...
def getVideoURL(keyword = "",num=1):
    ...
    for xpath in video_xpath:
        try:
            ele = b.find_element_by_xpath(str(xpath) % (1))
            print("找到第一个视频xpath")
            current_xpath = xpath
            break
        except:
    ...
    for i in range(num):
        b.switch_to.window(b.window_handles[1])
        try:
            ele = b.find_element_by_xpath(str(current_xpath) % (i + 1))

        except:# 若找不到则翻页
            try:
                ele = b.find_element_by_xpath(str('//*[@id="i_cecream"]/div[1]/div[1]/div[2]/div/div/div[3]/div/div/button[10]'))
            except:
                try:
                    ele = b.find_element_by_class_name('vui_button vui_pagenation--btn vui_pagenation--btn-side')
                except:
                    return url
            ele.click()
            time.sleep(5)
...
def crawl_bilibili(keyword : str,num : int):
    ...
    for i in range(len(url)):
        cmd = "you-get -o %s -f "%keyword.replace(' ', '_') + str(url[i])
        print(cmd)
        try:
            os.system(cmd)
        except:
